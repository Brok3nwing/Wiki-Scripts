...
def render_single_items():
    ...
    # print(id)
    for count, value in enumerate(range(len(id))):
        # print(f"Count: {count} - Range {range(len(id))}")
        slot = get_item_data_from_cache(id[value] - 512)
        # print(value, id[value] - 512)

        male_player_kit = f"{str(id[value]) if slot[0] == 0 else 0}," \
             f"{str(id[value]) if slot[0] == 1 else 0}," \
             f"{str(id[value]) if slot[0] == 2 else 0}," \
             f"{str(id[value]) if slot[0] == 3 else 0}," \
             f"{str(id[value]) if slot[0] == 4 else 363}," \
             f"{str(id[value]) if slot[0] == 5 else 0}," \
             f"{0 if slot[1] == 8 else 259}," \
             f"{0 if slot[1] == 6 else 342}," \
             f"{str(id[value]) if slot[0] == 7 else 356}," \
             f"{0 if slot[2] == 9 and slot[0] == 3 else str(id[value]) if slot[0] == 9 else 289}," \
             f"{str(id[value]) if slot[0] == 10 else 298}," \
             f"{0 if slot[2] == 11 else 270}"


        female_player_kit = f"{str(id[value]) if slot[0] == 0 else 0}," \
             f"{str(id[value]) if slot[0] == 1 else 0}," \
             f"{str(id[value]) if slot[0] == 2 else 0}," \
             f"{str(id[value]) if slot[0] == 3 else 0}," \
             f"{str(id[value]) if slot[0] == 4 else 312}," \
             f"{str(id[value]) if slot[0] == 5 else 0}," \
             f"{0 if slot[1] == 8 else 377}," \
             f"{0 if slot[1] == 6 else 317}," \
             f"{str(id[value]) if slot[0] == 7 else 326}," \
             f"{0 if slot[2] == 9 and slot[0] == 3 else str(id[value]) if slot[0] == 9 else 324}," \
             f"{str(id[value]) if slot[0] == 10 else 335}," \
             f"0"
        ...
        if slot[0] == 0:
            ...
            if not os.path.isfile(f"{out}/playerchathead/{get_item_name_from_cache(id[value] - 512)} chathead.png"):
                subprocess.call(head_proc)
                alt_name = name[value]
                name_from_cache = get_item_name_from_cache(id[value] - 512)
                ...
                if os.path.exists(end_name) or os.path.exists(alt_end_name):
                    ...
                    os.rename(f"{out}/playerchathead/[{male_player_kit.replace(',', ', ')}]_[{male_player_colors.replace(',', ', ')}].png",f"{out}/playerchathead/{get_item_name_from_cache(id[value] - 512)}{' chathead.png' if name[value] == '' else ' '+name[value]+' chathead.png'}")
        ...
        male_proc = f"java -jar E:/Renderer/renderer-all.jar --playerkit {male_player_kit} --playercolors {male_player_colors} --poseanim {808 if anim[value] == 0 else anim[value]} --yan2d {128 if rota[value] == 0 else rota[value]} --cache E:/Caches/{version[0]}/cache --out {out}/male"
        female_proc = f"java -jar E:/Renderer/renderer-all.jar --playerkit {female_player_kit} --playercolors {female_player_colors} --playerfemale --anim {808 if anim[value] == 0 else anim[value]} --yan2d {128 if rota[value] == 0 else rota[value]} --cache E:/Caches/{version[0]}/cache --out {out}/female"
        ...
        male_name = f"{get_item_name_from_cache(id[value] - 512)}{' equipped male.png' if name[value] == '' else ' '+str(name)[value]+' equipped male.png'}"
        female_name = f"{get_item_name_from_cache(id[value] - 512)}{' equipped female.png' if name[value] == '' else ' '+str(name)[value]+' equipped female.png'}"

        male_path = f"{out}/male/player/"
        female_path = f"{out}/female/player/"

        if read_data()[5][1] == "x":
            print("Rendering (male): ", get_item_name_from_cache(id[value] - 512))
            subprocess.call(male_proc)
            if os.path.isfile(os.path.join(male_path, male_name)):
                os.rename(pre_render_male_equipped_name, os.path.join(male_path, f"{count}_{male_name}"))
            else:
                os.rename(pre_render_male_equipped_name, os.path.join(male_path, male_name))

        if read_data()[5][2] == "x":
            print("Rendering (female): ", get_item_name_from_cache(id[value] - 512))
            subprocess.call(female_proc)
            if os.path.isfile(os.path.join(female_path, female_name)):
                os.rename(pre_render_female_equipped_name, os.path.join(female_path, f"{count}_{female_name}"))
            else:
                os.rename(pre_render_female_equipped_name, os.path.join(female_path, female_name))
