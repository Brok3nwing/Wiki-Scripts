...
version = "2022-07-06_a"

with open("versions/%s/worldMapDefinitions.json" % version) as f:
    defs = json.load(f)

with open("additional_defs.json") as f:
    defs += json.load(f)
overallXLow = 999
overallXHigh = 0
overallYLow = 999
overallYHigh = 0
for file in glob.glob("versions/%s/tiles/base/*.png" % version):
...
def getIconsInsideArea(plane, lowX, highX, lowY, highY, chunk_lowX=0, chunk_highX=7, chunk_lowY=0, chunk_highY=7, dx=0,
                       dy=0, dz=0, allPlanes=False):
    valid = []
    for icon in icons:
        if pointInsideBox(icon['position'], plane, lowX, highX, lowY, highY, chunk_lowX, chunk_highX, chunk_lowY,
                          chunk_highY, allPlanes):
            pos = icon['position']
    ...
    for plane in range(planes):
        ...
        for region in defn['regionList']:
            if 'xLowerLeft' in region:
                ...
                print
                oldLowX == newLowX, oldLowY == newLowY, oldHighX == newHighX, oldHighY == newHighY
                ...
                filename = "versions/%s/tiles/base/%s_%s_%s.png" % (
                version, region['oldPlane'] + plane, region['oldX'], region['oldY'])
                dx = region['newX'] * 64 + region['chunk_newXLow'] * 8 - region['oldX'] * 64 - region[
                    'chunk_oldXLow'] * 8
                dy = region['newY'] * 64 + region['chunk_newYLow'] * 8 - region['oldY'] * 64 - region[
                    'chunk_oldYLow'] * 8
                dz = 0 - region['oldPlane']
                validIcons.extend(
                    getIconsInsideArea(region['oldPlane'] + plane, region['oldX'], region['oldX'], region['oldY'],
                                       region['oldY'], region['chunk_oldXLow'], region['chunk_oldXHigh'],
                                       region['chunk_oldYLow'], region['chunk_oldYHigh'], dx, dy, dz,
                                       allPlanes=plane == 0))
                if os.path.exists(filename):
                    square = Image.open(filename)
                    ...
                    imY = (highY - region['newY']) * px_per_square * 64 + (
                                7 - region['chunk_newYHigh']) * px_per_square * 8
                    im.paste(cropped, box=(imX + 256, imY + 256))
            elif 'chunk_xLow' in region:
                validIcons.extend(
                    getIconsInsideArea(region['plane'] + plane, region['xLow'], region['xHigh'], region['yLow'],
                                       region['yHigh'], region['chunk_xLow'], region['chunk_xHigh'],
                                       region['chunk_yLow'], region['chunk_yHigh'], allPlanes=plane == 0))
                filename = "versions/%s/tiles/base/%s_%s_%s.png" % (
                version, region['plane'] + plane, region['xLow'], region['yLow'])
                if os.path.exists(filename):
                    square = Image.open(filename)
                    ...
                    imY = (highY - region['yLow']) * px_per_square * 64 + (
                                7 - region['chunk_yHigh']) * px_per_square * 8
                    im.paste(cropped, box=(imX + 256, imY + 256))
            elif 'xLow' in region:
                validIcons.extend(
                    getIconsInsideArea(region['plane'] + plane, region['xLow'], region['xHigh'], region['yLow'],
                                       region['yHigh'], allPlanes=plane == 0))
        ...
        for zoom in range(-3, 4):
            ...
            if zoom >= 0:
                for x, y, spriteId in validIcons:
                    ...
                    zoomed.paste(sprite, (imX + int(round(256 * scalingFactor)), int(round(imY + 256 * scalingFactor))),
                                 sprite)
            ...
            for x in range(lowZoomedX, highZoomedX + 1):
                for y in range(lowZoomedY, highZoomedY + 1):
                    ...
                    if not allBlack(cropped):
                        outfilename = "versions/%s/tiles/rendered/%s/%s/%s_%s_%s.png" % (
                        version, mapId, zoom, plane, x, y)
                        mkdir_p(outfilename)
